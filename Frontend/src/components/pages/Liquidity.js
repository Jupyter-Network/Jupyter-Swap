...
export default function Liquidity({ block, ethersProvider, routerContract }) {
  ...
  async function createLiquidityPool() {
    ...
    let value = token0Address == CONST.WBNB_ADDRESS ? newPosition[0] : 0n;
    value = token1Address == CONST.WBNB_ADDRESS ? newPosition[1] : value;

    transactions.push({
      transaction: routerContract.createPool,
      options: [token0Address, token1Address, startTick],
    });

    let gt = BigInt(createWidgetState.token0Address) > BigInt(createWidgetState.token1Address)

    let message = `<h3>Create New Pair:</h3> 
    <div style="background-color:white;border-radius:5px;text-align:end;padding:5px;">
    1 ${gt ? token0Symbol: token1Symbol}  <br/> = ${startPrice} ${gt ? token1Symbol : token0Symbol}
     </div>
        ...
        >
          ...
          <div>
            ...
            >
              ...
              <p>{blockData.poolBalance0.toString()}</p>
              <p>{blockData.poolBalance1.toString()}</p>
            </Container>
            <Container
              style={{ maxWidth: "96vw", width: 400, height: "fit-content" }}
            >
              <ContainerTitle>Your Positions</ContainerTitle>
              <p>Current tick {blockData.currentTick}</p>
              <LiquidityPositions
                blockData={blockData}
            ...
            <div style={{ width: "80%", margin: "0 auto" }}>
              <LabeledInput
                ...
                onChange={async (e) => {
                  ...
                  let reg = /[^\w*]/g;
                  const address = e.target.value.replaceAll(reg, "");
                  let symbol;
                  try {
              ...
              <p>
               {
                  createWidgetState.token0Symbol && createWidgetState.token1Symbol ? 
                  BigInt(createWidgetState.token0Address) < BigInt(createWidgetState.token1Address) ?
                      `1 ${createWidgetState.token0Symbol ? createWidgetState.token0Symbol:""} = ~ ${createWidgetState.startPrice} ${createWidgetState.token1Symbol ? createWidgetState.token1Symbol:""}`
                  :
                   `1 ${createWidgetState.token1Symbol ? createWidgetState.token1Symbol:""} = ~ ${createWidgetState.startPrice} ${createWidgetState.token0Symbol ? createWidgetState.token0Symbol:""}` : ""
                }
              </p>
